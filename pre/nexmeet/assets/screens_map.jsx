/* ===========================================================
   NexMeet — attendee live venue map + location gate + bottom nav
   "Location access -> dynamic map showing you and your matches"
   =========================================================== */
...
const MATCH_POS = [{ x: 66, y: 36, near: "Coffee Bar" }, { x: 26, y: 42, near: "Workshops" }, { x: 68, y: 66, near: "Demo Zone" }];

function dist(a, b) { return Math.round(Math.hypot(a.x - b.x, a.y - b.y) * 0.62); }

/* faint crowd */
const CROWD = (() => { let h = 7; const r = () => { h = (h * 1103515245 + 12345) & 0x7fffffff; return h / 0x7fffffff; }; return Array.from({ length: 26 }, () => ({ x: 8 + r() * 84, y: 6 + r() * 88 })); })();

/* ---------- LOCATION GATE ---------- */
function LocationGate({ onGrant, onSkip }) {
  return (
    <div className="scroll screen-enter" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "30px 32px 50px" }}>
      <div style={{ position: "relative", width: 120, height: 120, marginBottom: 30 }}>
        ...
        <div style={{ position: "absolute", inset: 24, borderRadius: "50%", background: "var(--accent)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 12px 30px color-mix(in srgb, var(--accent) 40%, transparent)" }}><Icon name="location" size={40} /></div>
      </div>
...
/* ---------- MAP ---------- */
function MapScreen({ you, matches, granted, onGrant, onSkip, onOpenMatch }) {
  const [sel, setSel] = useMp(null);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div className="grow" style={{ position: "relative", margin: "0 16px", borderRadius: 24, overflow: "hidden", background: "var(--card)", border: "1px solid var(--card-edge)" }}>
        ...
        {/* match pins (HTML overlay for crisp avatars) */}
        {mm.map((m, i) => (
      ...
      <div style={{ flex: "0 0 auto", padding: "14px 16px 18px" }}>
        {selected ? (
          <div className="screen-enter" style={{ background: selected.accent, borderRadius: 20, padding: 16, color: "#fff", boxShadow: "var(--shadow-md)" }}>
            ...
            <button className="btn btn-full" onClick={() => onOpenMatch(selected)} style={{ background: "rgba(255,255,255,.18)", color: "#fff", marginTop: 12, minHeight: 46 }}>See why you matched <Icon name="arrow" size={17} /></button>
          </div>
        ) : (
...
function BottomNav({ active, onNav }) {
  const tabs = [{ id: "matches", label: "Matches", icon: "spark" }, { id: "map", label: "Live map", icon: "location" }];
  return (
...
Object.assign(window, { MapScreen, LocationGate, BottomNav });
