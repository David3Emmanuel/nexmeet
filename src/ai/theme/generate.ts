...
export async function generateTheme(flyerUrl: string): Promise<GeneratedTheme> {
  try {
    ...
    return {
      background,
      foreground: ensureReadable(
        raw.foreground ?? FALLBACK.foreground,
        background,
      ),
      accent,
      accentForeground: ensureReadable(
        raw.accentForeground ?? FALLBACK.accentForeground,
        accent,
      ),
