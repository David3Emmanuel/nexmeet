/* ===========================================================
   NexMeet — seed data + matching engine (ES module)
   =========================================================== */
...
export const SKILL_BANK = [
  "Product", "Design", "Frontend", "Backend", "ML / AI", "Growth",
  "Data", "Mobile", "DevOps", "Fundraising", "Sales", "Research", "Hardware", "Content",
];

export const AV = [
  "#FF5A2C", "#3A2E58", "#1F5C45", "#2F6BFF", "#C0392B", "#0E7C86", "#B7791F", "#7A3E9D",
];

// Fixed palette constants (replaces getComputedStyle CSS var reads)
export const COLORS = {
  coral: "#FF5A2C",
  plum: "#3A2E58",
  forest: "#1F5C45",
...
function reasonFor(you: YouProfile, p: Person): string {
...
function conversationStarter(you: YouProfile, p: Person): string {
  const f = p.name.split(" ")[0];
  const me = you.name.split(" ")[0];
  const starters = [
    `“Hey ${f} — I heard you're ${lower(p.focus).replace(/\.$/, "")}. How did you get into that?”`,
    `“${f}? I'm ${me}. ${p.fun} — I have to ask about that.”`,
...
function score(you: YouProfile, p: Person): number {
  let s = 0;
...
export function generateMatches(you: YouProfile, pool: Person[]): Match[] {
  const accents = [COLORS.coral, COLORS.plum, COLORS.forest];
  const ranked = pool
...
const CONTACTS: Record<string, string> = {
  a01: "@ada.builds · ada@paystack.com",
  a02: "@tobi_a · +234 803 555 0142",
  a03: "@zainab.designs",
  a04: "@emeka_ml · emeka@google.com",
  a05: "@chiamaka.codes · +234 701 555 0199",
  a06: "@danielmensah_vc",
  a07: "@aisha.grows · aisha@freelance.co",
  a08: "@kelvin.builds",
  a09: "@fatima.data · +234 802 555 0177",
  a10: "@seyi_pm · seyi@moniepoint.com",
  a11: "@ngozi.agrisense",
  a12: "@bencarter · ben@vercel.com",
};

export const SEED: Person[] = SEED_BASE.map(p => ({
  ...p,
  contact: CONTACTS[p.id] || ("@" + p.name.split(" ")[0].toLowerCase()),
}));

export const DEMO_YOU: YouProfile = {
  name: "Moni Adeleke",
