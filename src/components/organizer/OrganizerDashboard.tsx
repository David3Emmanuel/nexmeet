...
function ConnectionGraph({ nodes = 14 }: { nodes?: number }) {
  const W = 560, H = 360;
  const ptsRef = useRef<{ x: number; y: number; r: number; c: string }[]>([]);
  if (ptsRef.current.length !== nodes) {
    let h = 99;
    const rnd = () => { h = (h * 1103515245 + 12345) & 0x7fffffff; return h / 0x7fffffff; };
    ptsRef.current = Array.from({ length: nodes }, (_, i) => ({
...
export default function OrganizerDashboard({ event, onExit, onNewEvent, onHome }: OrganizerDashboardProps) {
                  ...
                  <tr key={p.id} style={{ borderTop: "1px solid var(--line)" }}>
                    ...
                    <td style={{ padding: "12px 10px" }}>
                      <span style={{ fontSize: 12.5, fontWeight: 700, padding: "4px 10px", borderRadius: 999, background: "color-mix(in srgb, var(--accent) 12%, transparent)", color: "var(--accent)" }}>
                        {(GOALS.find(g => g.id === p.goal) || { label: p.goal }).label}
                      </span>
                    </td>
                    <td style={{ padding: "12px 10px" }}>
